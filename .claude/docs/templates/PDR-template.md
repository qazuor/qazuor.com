...
**Date**: YYYY-MM-DD **Status**: Draft | In Review | Approved | In Progress |
Completed **Priority**: P0 (Critical) | P1 (High) | P2 (Medium) | P3 (Low)
**Owner**: {Name/Team} **Feature Type**: New Feature | Enhancement | Bug Fix |
Refactor
...
{Clear, concise description of the problem we're solving. Why does this matter?
Who is affected?}
...
**As a** {user type/role} **I want to** {action/capability} **So that**
{benefit/value}
...
**Description:** {Explain the user flow in detail}
...
_Desktop mockup showing {brief description of what's in the mockup}. {Key design
decisions or highlights}._
...
_Mobile-optimized version with {responsive adaptations}. Touch-friendly targets
(minimum 44x44px)._
...
_Component mockup showing all interactive states: default, hover, active,
disabled, error, and loading._
...
- **When to include mockups**: Complex flows, novel patterns, specific layout
  requirements, user request
- **When to skip mockups**: Simple layouts, standard components, unclear
  requirements
- **Caption format**: `![Description](./mockups/filename-YYYYMMDD-HHMMSS.png)` +
  italic caption
- **File location**: `./mockups/` folder in session directory
- **Naming convention**: `{feature}-{device}-YYYYMMDD-HHMMSS.png`
- **Cost awareness**: Use judiciously (2-4 key mockups per feature, flux-schnell
  model default)
...
**Exceptions:** {Any exceptions and justification}
...
**Justification:** {Why these dependencies are necessary and alternatives
considered}
...
**Q1**: {Question} **A1**: {Answer} - {Date} - {Who answered}

**Q2**: {Question} **A2**: {Answer} - {Date} - {Who answered}

---

## Notes

**General Notes:** {Any additional context, decisions, or information that
...
**Status**: This PDR is {Draft/In Review/Approved/In Progress/Completed} **Next
Actions**: {What needs to happen next} **Owner**: {Who is responsible for next
steps}
